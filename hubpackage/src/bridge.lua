--[[
  ...
  SmartThings Edge driver module for handling interface with Edgebridge Server
  
  Common module for LAN Motion, LAN Presence, LAN Trigger drivers
...
local function init_clientsocket()
  ...
  local clientsock = assert(socket.tcp(), "create TCP socket")
...
local function watch_socket(sock)
...
local function start_bridge_server(driver, triggerfunc)
  ...
  cosock.spawn(function()
      while true and (shutdownflag == false) do
        watch_socket(serversock)
      end
    end)
