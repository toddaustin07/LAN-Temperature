--[[
  ...
  LAN Temperature/Humidity Driver for LAN-based devices/apps that can send an HTTP message with temperature or humidity
...
local function trigger_callback(devaddr, method, endpoint)
  ...
  if (method == 'POST') or (method == 'Post') or (method == 'post') then

    local pathparts = split_path(endpoint)
          
    local name = pathparts[1]
    local cmd = pathparts[2]
    local numvalue = tonumber(pathparts[3])
    ...
    for _, device in ipairs(device_list) do
      ...
      if device.preferences.deviceaddr ==  devaddr then
        ...
        if device.preferences.devicename == name then
          ...
          log.info (string.format('Message from %s: command=%s, state=%s', name, cmd, numvalue))
          foundflag = true
          
          if numvalue then
          
            if cmd == 'temperature' then
              device:emit_event(capabilities.temperatureMeasurement.temperature({value=numvalue, unit=device.preferences.rtempunit}))
            
            elseif cmd == 'humidity' then
              device:emit_event(capabilities.relativeHumidityMeasurement.humidity(numvalue))
            ...
            log.error ('No numeric value found:', pathparts[3])
          end
        end
      end
    end
...
local function create_device(driver)
  ...
  local MODEL = 'LAN Temp&Humidity Device'
  local VEND_LABEL = 'LAN Temp&Humidity Device'
  local ID = 'LANPresence_' .. socket.gettime()
  local PROFILE = 'lantemp.v1'
...
log.info ('LAN Temp&Humidity Driver v1.0 Started')
